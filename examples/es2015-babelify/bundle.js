...
MixpanelLib.prototype._send_request = function (url, data, callback) {
  ...
  if (!_utils.window.navigator.onLine) return url;
...
MixpanelLib.prototype.track = (0, _gdprUtils.addOptOutCheckMixpanelLib)(function (event_name, properties, callback) {
  ...
  var preURL = this.get_config('api_host') + '/track/';
  var preData = { 'data': encoded_data };

  var online = _utils.window.navigator.onLine;

  var send_result = this._send_request(preURL, preData, this._prepare_callback(callback, truncated_data));

  return online ? truncated_data : send_result;
});
 ...
 */
MixpanelLib.prototype.identify = function (unique_id, _set_callback, _add_callback, _append_callback, _set_once_callback, _union_callback, _unset_callback, _remove_callback) {
  // Optional Parameters
